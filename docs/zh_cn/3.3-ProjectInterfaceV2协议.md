...
| 2026-2-23 | v2.3.0 | 新增 `checkbox` 多选类型、`option.controller`、全局/resource/controller 级 option |
...
- task `object[]`  
  ...
  - controller `string[]`  
    ...
    当用户选择了某个控制器时，只有支持该控制器的任务才会显示在用户界面中供选择。这允许为不同控制器类型提供专门的任务配置，比如某些任务只适用于 Adb 控制器，某些任务只适用于 Win32 控制器。
...
- option `record<string, object>`  
  ...
  - controller `string[]` **💡 v2.3.0**  
    ...
    当用户选择了某个控制器时，只有适用于该控制器的配置项才会显示在用户界面中供配置。
  ...
  - cases `object[]`  
    ...
    **注意：** 当 `type` 为 `"checkbox"` 时，用户可以同时选中多个 case，所有被选中的 case 的 `pipeline_override` 会依次合并生效。
  ...
  - default_case `string`  
    默认选项名称，仅在 `type` 为 `"select"` / `"switch"` 时使用。Client 可以使用该值作为选项的初始选中值。可选。

  - default_cases `string[]` **💡 v2.3.0**  
    默认选中的选项名称列表，仅在 `type` 为 `"checkbox"` 时使用。Client 可以使用该值作为多选框的初始选中值。可选。
...
```jsonc
{
  "option": {
    "启用功能": {
      "type": "checkbox",
      "label": "$启用功能",
      "description": "选择要启用的功能模块，可多选",
      "default_cases": ["自动战斗", "自动拾取"],
      "cases": [
        {
          "name": "自动战斗",
          "label": "$自动战斗",
          "description": "启用自动战斗功能",
          "pipeline_override": {
            "AutoBattle": {
              "enabled": true
            }
...
6. 根据 `display` 指定的渠道（可多个）将处理后的文本展示给用户

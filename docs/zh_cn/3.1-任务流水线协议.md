...
- `anchor`: *string* | *list<string, >* | *object*  
    ...
    - **对象**：`"anchor": {"A": "TargetNode", "B": ""}` - 将锚点 A 设置为 TargetNode，锚点 B 清除（空字符串表示清除该锚点）
...
- `roi`: *array<int, 4>* | *string*  
  ...
  - *array<int, 4>*: 识别区域坐标 [x, y, w, h]。支持负数：x/y 负数表示从右/下边缘计算；w/h 为 0 表示延伸至边缘，为负数时取绝对值并将 (x, y) 视为右下角。
...
- `target`: *true* | *string* | *array<int, 2>* | *array<int, 4>*  
  ...
  - *array<int, 2>*: 固定坐标点 [x, y]。支持负数，表示从右/下边缘计算。  
  - *array<int, 4>*: 固定坐标区域 [x, y, w, h]，在矩形内随机选点（中心概率高）。支持负数，含义同 `roi`。
...
在节点中添加 `focus` 字段，键为消息类型，值为要展示的内容配置。

每个消息类型的值支持两种写法：

- **字符串**（旧写法，向后兼容）：直接作为展示内容，等价于 `display: "log"`
- **对象**（新写法）：通过 `content` 指定内容，通过 `display` 指定展示位置

模板字符串支持文件路径、URL 或直接文本，内容支持 Markdown 格式，支持国际化（以 `$` 开头）。模板中可使用 `{字段名}` 格式的占位符，UI 会自动替换为实际值。

```jsonc
{
  "NodeA": {
    "focus": {
      // 旧写法：string，默认显示在运行日志（向后兼容）
      "Node.Recognition.Succeeded": "{name} 识别命中，准备开始执行",

      // 新写法：object，通过 display 指定展示位置
      "Node.Action.Starting": {
        "content": "{name} 开始执行，任务 ID: {task_id}",
        "display": "toast"
      },

      // display 支持数组，同时发送到多个展示位置
      "Node.Action.Failed": {
        "content": "❌ {name} 执行失败！",
        "display": ["log", "modal"]
      }
...
| --- | --- |
| `"log"` | 默认，显示在运行日志中 |
| `"toast"` | **应用内**轻提示，悬浮在 UI 内部，自动消失，不阻断流程 |
| `"notification"` | **系统级**通知，发送到操作系统通知栏/托盘，适合后台运行时告知用户 |
| `"modal"` | **阻塞式**弹窗，不关闭则不允许用户进行其他操作 |
| `"dialog"` | **非阻塞式**对话框，展示消息，用户可忽略 |

`display` 可以是 `string` 或 `array<string>`，不填时等价于 `"log"`。
